...
document.addEventListener('pointerdown',  ptrDownHandler, false);

function ptrDownHandler(e) {
  e.preventDefault();

  document.addEventListener('pointermove',  ptrMoveHandler, false);
  document.addEventListener('pointerup',    ptrUpHandler, false);
...
function ptrMoveHandler(e) {
  if (e.target.classList.contains('clone') == false)
    return ;
